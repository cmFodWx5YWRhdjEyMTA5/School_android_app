...
import com.microbotic.temperature.utils.ShellUtil;
...
public class CheckEthernetService extends IntentService {
    ...
    private static final String CHECK_STRING = "192.168.99";

    @Override
    protected void onHandleIntent(Intent intent) {
//        Csjlogger.debug("======================================" + CHECK_STRING);

        while (!(getEthernetIP() != null && getEthernetIP().contains(CHECK_STRING))) {
            if (isConnected) {
                break;
            }
            upDownEht0(CLOSE);
    ...
    public void onCreate() {
        ...
        filter.addAction("com.csjbot.blackgaga.connected");
...
//        }
    ...
    private String getEthernetIP() {
        ...
        try {
            ...
            if (status != 0) {
                Log.e(CheckEthernetService.class.getSimpleName(), "root error status is " + status);
